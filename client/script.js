        // Import gradient manager
        import('./gradient.js').then(({ initGradient, destroyGradient }) => {
            // Store functions for later use
            window.gradientManager = { initGradient, destroyGradient };
        }).catch(err => {
            console.warn('Failed to load gradient manager:', err);
        });
        ...
        function buildTrackCard(track, list, index) {
            ...
            card.addEventListener("click", function() { currentQueue = list; currentIndex = index; playTrack(track); });
            return card;
        }
        ...
        function playByIndex(index, fromRepeat) {
            ...
            if (shuffle && !fromRepeat) {
                var randIndex = Math.floor(Math.random() * currentQueue.length);
