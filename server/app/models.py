...
class User(Base):
    ...
    followed_artists = relationship("FollowedArtist", back_populates="user", cascade="all, delete-orphan")
...
class Playlist(Base):
    ...
    followers = relationship(
        "FollowedPlaylist", back_populates="playlist", cascade="all, delete-orphan"
    )
...
class FollowedPlaylist(Base):
    __tablename__ = "followed_playlists"
    ...
    playlist_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("playlists.id", ondelete="CASCADE"), index=True
    )
    ...
    __table_args__ = (
        UniqueConstraint("user_hash", "playlist_id", name="uq_user_followed_playlist"),
    )
...
class FollowedArtist(Base):
    ...
    artist_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("artists.id", ondelete="CASCADE"), index=True
    )
    followed_at: Mapped[datetime] = mapped_column(
        DateTime, default=lambda: datetime.now(timezone.utc)
    )
    ...
    __table_args__ = (
        UniqueConstraint("user_hash", "artist_id", name="uq_user_followed_artist"),
    )
